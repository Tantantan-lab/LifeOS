...
const METRIC_VALUE_LABELS: Record<string, (v: number, e: LifeEvent) => string> = {
  ...
  "health.workout.session": (v) => `${v} session${v === 1 ? "" : "s"}`,
  "health.workout.minutes": (v) => formatDuration(v),
...
const METRIC_LABELS: Record<string, (e: LifeEvent) => string> = {
  ...
  "health.workout.session": (e) => String(e.metadata.title ?? e.metadata.type ?? "Workout"),
  "health.workout.minutes": () => "Workout time",
