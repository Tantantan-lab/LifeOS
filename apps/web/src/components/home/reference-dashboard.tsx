...
function Heatmap({ days, stats, filter }: { days: HeatmapDay[]; stats: HeatmapStats; filter: HeatFilter }) {
  ...
  return (
    <div className="mt-5 overflow-hidden">
      ...
      <div className="mt-5 flex items-end justify-between">
        <div className="flex gap-12">{[[String(stats.activeDays90d), "active days"], [String(stats.currentStreak), "day streak"], [String(stats.longestStreak), "longest streak"]].map(([value, label]) => <div key={label}><div className="text-[17px] font-semibold text-white">{value}</div><div className="text-[11px] text-[#8d96a6]">{label}</div></div>)}</div>
        <div className="flex items-center gap-1 text-[10px] text-[#929aaa]"><span className="mr-1">Less</span>{[0, 1, 2, 3, 4].map((level) => <span key={level} className="size-3 rounded-[3px]" data-heat-level={level} />)}<span className="ml-1">More</span></div>
      </div>
    </div>
  );
}
  ...
  return (
    <Panel className="today-card">
      ...
      <div className="mt-3 grid grid-cols-[154px_1fr] items-center gap-6">
        ...
        <div className="space-y-3">{items.length === 0 ? <div className="text-[12px] text-[#8e98a8]">{locale === "zh" ? "今天还没有记录" : "No entries yet today"}</div> : items.slice(0, 5).map((item, index) => <div key={`${item.time}-${item.metricLabel}-${index}`} className="grid grid-cols-[20px_1fr_auto] items-center gap-2 text-[13px]"><span className={`today-check accent-${domainAccent[item.domain]}`}><Check className="size-3" /></span><span className="truncate text-[#c5cbd6]">{item.metricLabel}</span><span className="text-[#e8ebf0]">{item.valueLabel}</span></div>)}</div>
      </div>
      {action && <div className="mt-4 grid grid-cols-[44px_1fr_34px] items-start gap-3 rounded-[10px] bg-[#151b25] p-3.5">
        ...
        <div><div className="text-[11px] text-[#9ea7b5]">{locale === "zh" ? "下一步行动" : "Next Best Action"}</div><div className="mt-1 font-semibold text-white">{action.title}</div><div className="text-[12px] text-[#b2bac7]">{action.minutes} min</div><div className="mt-1 line-clamp-2 text-[11px] leading-4 text-[#8e98a8]">{locale === "zh" ? "原因：" : "Reason: "}{action.reason}</div>{logged && <div className="mt-1 text-[11px] text-[#52db89]">{locale === "zh" ? "已记录" : "Logged"}</div>}</div>
...
function MeVsMe({ windows, locale }: { windows: Record<WindowKey, MeVsMeWindow>; locale: "en" | "zh" }) {
  ...
  return (
    <Panel className="bottom-card">
      ...
      <div className="mt-3 space-y-3.5">{rows.map((row) => { const Icon = domainIcon[row.domain]; const accent = domainAccent[row.domain]; return <div key={row.key} className="grid grid-cols-[1fr_82px_22px_82px_58px] items-center text-[12px]"><span className={`flex items-center gap-3 accent-${accent}`}><Icon className="size-4 text-[var(--accent)]" /><span className="truncate text-[#d6dae2]">{row.label}</span></span><span>{row.thenLabel}</span><span className="text-[#667080]">→</span><span>{row.nowLabel}</span><span className={row.deltaPct < 0 ? "text-[#ef766c]" : "text-[#52db89]"}>{pct(row.deltaPct)}</span></div>; })}</div>
    </Panel>
  );
}
...
function InsightsCard({ insight, trends, gaps, locale }: { insight: InsightRecord | null; trends: InsightTrendRow[]; gaps: GapItem[]; locale: "en" | "zh" }) {
  ...
  return (
    <Panel className="bottom-card">
      ...
      {tab === "summary" && <><div className="mt-3 rounded-[10px] bg-[#151a23]"><div className="flex gap-3 p-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-[#173026] text-[#50d88a]"><ArrowUp className="size-4" /></span><div><div className="text-[12px] font-medium text-white">{locale === "zh" ? "这个月你取得了持续进展。" : "You made solid progress this month."}</div><p className="mt-1 text-[11px] leading-5 text-[#a9b1bf]">{lead ? `${lead.label} ${locale === "zh" ? "近 30 天提升" : "improved by"} ${pct(lead.delta30)}.` : (locale === "zh" ? "继续记录数据以生成趋势。" : "Keep logging data to reveal trends.")}</p></div></div><div className="grid grid-cols-4 border-t border-[#252b35] py-3 text-center">{trends.slice(0, 4).map((row) => <div key={row.metric} className="border-r border-[#262c36] last:border-0"><div className={row.delta30 < 0 ? "text-[#ef766c]" : "text-[#54d88a]"}>{pct(row.delta30)}</div><div className="mt-0.5 truncate px-1 text-[9px] text-[#a1a9b6]">{row.label}</div></div>)}</div></div><div className="mt-3 flex items-center gap-3 rounded-[10px] bg-[#151a23] p-3"><span className="flex size-9 items-center justify-center rounded-full bg-[#2c1d3d] text-[#b96cf5]"><Target className="size-4" /></span><div className="flex-1"><div className="text-[10px] text-[#949dac]">{locale === "zh" ? "首要差距" : "Top Gap"}</div><div className="text-[12px] font-medium text-white">{gaps[0]?.skill ?? "—"}</div></div><ChevronRight className="size-4 text-[#b9c0cb]" /></div></>}
      {tab === "trends" && <div className="mt-3 space-y-2 rounded-[10px] bg-[#151a23] p-3">{trends.slice(0, 5).map((row) => <div key={row.metric} className="flex items-center justify-between text-[11px]"><span className="text-[#b3bbc7]">{row.label}</span><span className={row.delta30 < 0 ? "text-[#ef766c]" : "text-[#54d88a]"}>{pct(row.delta30)}</span></div>)}</div>}
