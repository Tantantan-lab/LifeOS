...
function SmallLink({ href = "#", children = "View more" }: { href?: string; children?: React.ReactNode }) {
  return <Link href={href} className="inline-flex items-center gap-1 text-[11px] text-[var(--dash-link)] transition hover:text-[#9bb0ff]">{children} <ArrowRight className="size-3" /></Link>;
}
...
function ActivityCard({ card }: { card: (typeof activityCards)[number] }) {
  ...
  return (
    <Panel className={`activity-card accent-${card.accent}`}>
      <div className="flex items-center gap-2 text-[13px] text-[var(--dash-fg-2)]"><Icon className="size-[19px] text-[var(--accent)]" strokeWidth={1.9} /><span>{card.label}</span></div>
      <div className="mt-3 text-[24px] font-semibold leading-none tracking-[-0.025em] text-[var(--dash-fg)]">{card.value}</div>
      <div className="mt-2 flex items-center gap-1 text-[13px] text-[var(--dash-positive)]"><ArrowUp className="size-3.5" /> {card.change}</div>
...
function Heatmap({ days, stats, filter, gridStart, locale }: { days: HeatmapDay[]; stats: HeatmapStats; filter: HeatFilter; gridStart: string; locale: "en" | "zh" }) {
  ...
  return (
    <div className="mt-5 overflow-hidden">
      ...
      <div
        className="ml-10 grid text-[11px] text-[var(--dash-fg-3)]"
      ...
      <div className="mt-3 flex gap-3">
        <div className="grid h-[116px] w-7 shrink-0 grid-rows-7 gap-[3px] text-[11px] leading-none text-[var(--dash-fg-3)]">{dayRows.map((day) => <span key={day} className="flex items-center">{locale === "zh" ? ({ Mon: "一", Tue: "二", Wed: "三", Thu: "四", Fri: "五", Sat: "六", Sun: "日" }[day] ?? day) : day}</span>)}</div>
      ...
      {selectedDay && (
        <div className="mt-3 rounded-[10px] bg-[var(--dash-surface)] p-4">
      ...
      <div className="mt-5 flex items-end justify-between">
        <div className="flex gap-12">{[[String(stats.activeDays90d), "active days"], [String(stats.currentStreak), "day streak"], [String(stats.longestStreak), "longest streak"]].map(([value, label]) => <div key={label}><div className="text-[17px] font-semibold text-[var(--dash-fg)]">{value}</div><div className="text-[11px] text-[var(--dash-fg-3)]">{locale === "zh" ? (zhStreak[label] ?? label) : label}</div></div>)}</div>
        <div className="flex items-center gap-1 text-[10px] text-[var(--dash-fg-3)]"><span className="mr-1">{locale === "zh" ? "少" : "Less"}</span>{[0, 1, 2, 3, 4].map((level) => <span key={level} className="size-3 rounded-[3px]" data-heat-level={level} />)}<span className="ml-1">{locale === "zh" ? "多" : "More"}</span></div>
      </div>
    </div>
  );
}
...
function TodayCard({ items, completion, action, locale }: { items: TodayItem[]; completion: number | null; action: NextBestAction | null; locale: "en" | "zh" }) {
  ...
  return (
    <Panel className="today-card">
      <div className="flex items-center justify-between"><h2 className="text-[18px] font-semibold text-[var(--dash-fg)]">{locale === "zh" ? "今天" : "Today"}</h2><SmallLink href="/today">{locale === "zh" ? "查看详情" : "View details"}</SmallLink></div>
      <div className="mt-3 grid grid-cols-[154px_1fr] items-center gap-6">
        ...
        <div className="space-y-3">{items.length === 0 ? <div className="text-[12px] text-[var(--dash-fg-3)]">{locale === "zh" ? "今天还没有记录" : "No entries yet today"}</div> : items.slice(0, 5).map((item, index) => <div key={`${item.time}-${item.metricLabel}-${index}`} className="grid grid-cols-[20px_1fr_auto] items-center gap-2 text-[13px]"><span className={`today-check accent-${domainAccent[item.domain]}`}><Check className="size-3" /></span><span className="truncate text-[var(--dash-fg-2)]">{locale === "zh" ? zhLabel(item.metricLabel) : item.metricLabel}</span><span className="text-[var(--dash-fg-2)]">{locale === "zh" ? zhValue(item.valueLabel) : item.valueLabel}</span></div>)}</div>
      </div>
      {action && <div className="mt-4 grid grid-cols-[44px_1fr_34px] items-start gap-3 rounded-[10px] bg-[var(--dash-surface)] p-3.5">
        <span className="flex size-10 items-center justify-center rounded-[9px] bg-gradient-to-br from-[#376eff] to-[#6675ff] text-[var(--dash-fg)] shadow-[0_8px_20px_#3162ff33]"><PenLine className="size-4" /></span>
        <div><div className="text-[11px] text-[var(--dash-fg-3)]">{locale === "zh" ? "下一步行动" : "Next Best Action"}</div><div className="mt-1 font-semibold text-[var(--dash-fg)]">{locale === "zh" ? `${zhLabel(action.label)} · ${action.isDuration ? `+${action.extra} 分钟` : `再 +${action.extra}`}` : action.title}</div><div className="text-[12px] text-[var(--dash-fg-2)]">{action.isDuration ? `${action.minutes} min` : action.targetLabel}</div><div className="mt-1 line-clamp-2 text-[11px] leading-4 text-[var(--dash-fg-3)]">{locale === "zh" ? `原因：${zhLabel(action.label)}近 30 天平均 ${action.currentLabel}，目标 ${action.targetLabel}。` : "Reason: " + action.reason}</div>{logged && <div className="mt-1 text-[11px] text-[var(--dash-positive)]">{locale === "zh" ? "已记录" : "Logged"}</div>}</div>
        <button onClick={completeAction} disabled={isPending || logged || !action.isDuration} className="mt-1 flex size-8 items-center justify-center rounded-full bg-[var(--dash-surface-hi)] text-[var(--dash-fg)] disabled:opacity-50" aria-label={locale === "zh" ? "记录下一步行动" : "Log next action"}>{logged ? <Check className="size-4" /> : <ArrowRight className="size-4" />}</button>
      </div>}
    </Panel>
  );
}
...
function MeVsMe({ windows, locale }: { windows: Record<WindowKey, MeVsMeWindow>; locale: "en" | "zh" }) {
  ...
  return (
    <Panel className="bottom-card">
      ...
      <div className="mt-4 grid grid-cols-[1fr_82px_22px_82px_58px] items-end text-[10px] text-[var(--dash-fg-3)]"><span /><span>{locale === "zh" ? "过去" : "Then"}<br />({locale === "zh" ? "对比期" : "previous"})</span><span /><span>{locale === "zh" ? "现在" : "Now"}</span><span>{locale === "zh" ? "变化" : "Change"}</span></div>
      <div className="mt-3 space-y-3.5">{rows.map((row) => { const Icon = domainIcon[row.domain]; const accent = domainAccent[row.domain]; return <div key={row.key} className="grid grid-cols-[1fr_82px_22px_82px_58px] items-center text-[12px]"><span className={`flex items-center gap-3 accent-${accent}`}><Icon className="size-4 text-[var(--accent)]" /><span className="truncate text-[var(--dash-fg-2)]">{locale === "zh" ? zhLabel(row.label) : row.label}</span></span><span>{row.thenLabel}</span><span className="text-[#667080]">→</span><span>{row.nowLabel}</span><span className={row.deltaPct < 0 ? "text-[var(--dash-negative)]" : "text-[var(--dash-positive)]"}>{pct(row.deltaPct)}</span></div>; })}</div>
    </Panel>
  );
}
...
function GoalsCard({ progress, locale }: { progress: GoalProgress; locale: "en" | "zh" }) {
  return (
    <Panel className="bottom-card">
      ...
      <div className="mt-5 flex items-center gap-3"><span className="flex size-9 items-center justify-center rounded-[9px] bg-[var(--dash-surface-hi)] text-[#5687ff]"><Target className="size-[17px]" /></span><div className="min-w-0 flex-1"><div className="text-[12px] font-medium text-[var(--dash-fg)]">{locale === "zh" ? zhLabel(progress.targetLabel) : progress.targetLabel}</div><div className="text-[11px] text-[var(--dash-fg)]">{locale === "zh" ? "等级" : "Level"} {progress.level}</div></div></div>
      <div className="ml-12 mt-2 flex items-center gap-3"><span className="h-2 flex-1 overflow-hidden rounded-full bg-[var(--dash-ring-track)]"><span className="block h-full rounded-full bg-gradient-to-r from-[#4383ff] to-[#666eff]" style={{ width: `${progress.overall ?? 0}%` }} /></span><span className="text-[11px]">{progress.overall ?? "—"}%</span></div>
      <div className="mt-4 space-y-2.5">{progress.skills.map((skill) => { const done = skill.status === "Completed" || skill.status === "Proficient"; return <div key={skill.skill} className="grid grid-cols-[16px_1fr_88px_18px_92px] items-center gap-2 text-[11px]"><span className={done ? "text-[var(--dash-positive)]" : "text-[var(--dash-fg-3)]"}>{done ? <Check className="size-4" /> : <Circle className="size-4" />}</span><span className="text-[var(--dash-fg-2)]">{locale === "zh" ? zhLabel(skill.skill) : skill.skill}</span><span>{skill.score === null ? "—" : `${skill.score} / ${skill.target}`}</span><span className={done ? "text-[var(--dash-positive)]" : "text-[var(--dash-fg-3)]"}>{done ? <Check className="size-4" /> : <Circle className="size-4" />}</span><span className="text-[var(--dash-fg-2)]">{locale === "zh" ? zhStatus[skill.status] ?? skill.status : skill.status}</span></div>; })}</div>
    </Panel>
  );
}
      ...
      <header className="dashboard-topbar">
        <div>
          ...
          {data.header.top && Math.abs(data.header.top.deltaPct) > 0 && (
            <p className="mt-1 text-[12px] text-[var(--dash-fg-3)]">
              {locale === "zh"
