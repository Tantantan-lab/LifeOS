...
async function main() {
  ...
  await checkPage("/", [
    ...
    ["html owns the page background (root-canvas repaint fix)", `getComputedStyle(document.documentElement).backgroundColor === "rgb(9, 13, 18)"`],
    ...
    ["date chevrons shift the label and restore", `(async () => { const controls = document.querySelector('.dashboard-date-controls'); if (!controls) return false; const label = controls.querySelector('span'); const btns = controls.querySelectorAll('button'); const before = label.textContent; btns[0].click(); await new Promise(r => setTimeout(r, 300)); const afterLeft = label.textContent; btns[1].click(); await new Promise(r => setTimeout(r, 300)); const restored = label.textContent === before; label.click(); return afterLeft !== before && restored; })()`],
    ["heatmap cell click drives the date label", `(async () => { const cells = Array.from(document.querySelectorAll('.heatmap-grid button')).filter(b => ['1','2','3','4'].includes(b.dataset.level)); if (!cells.length) return false; const label = document.querySelector('.dashboard-date-controls span'); const before = label.textContent; const cell = cells[0]; cell.click(); await new Promise(r => setTimeout(r, 300)); const changed = label.textContent !== before; cell.click(); await new Promise(r => setTimeout(r, 300)); return changed && label.textContent === before; })()`],
  ]);
  ...
  await checkPage("/today", [
    ...
    ["connector rows show no synthetic clock", `(() => { const ol = document.querySelector('ol'); if (!ol) return true; return !ol.innerText.includes("23:00"); })()`],
  ]);
  ...
  await checkPage("/insights?tab=analysis", [
    ["analysis tab renders four sections", `["FACT","TREND","GAP","ACTION"].every(h => document.body.innerText.toUpperCase().includes(h))`],
    ["insight is typesafe-selected", `document.body.innerText.includes("typesafe") && !document.body.innerText.includes("No weekly insight yet")`],
    ["no judgment words in content", `!document.body.innerText.match(/\\b(bad|poor|fail|worst|missed|percentile)\\b/i)`],
    ["zh mode: insight content translated", `(async () => { const toggle = document.querySelector('.language-toggle'); if (!toggle) return false; toggle.click(); await new Promise(r => setTimeout(r, 400)); const text = document.body.innerText; const labels = ['事实','趋势','差距','行动'].every(l => text.includes(l)); const cjk = Array.from(document.querySelectorAll('p')).filter(p => /[\\u4e00-\\u9fff]/.test(p.textContent)).length >= 4; toggle.click(); await new Promise(r => setTimeout(r, 400)); return labels && cjk; })()`],
  ]);
