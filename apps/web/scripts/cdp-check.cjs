...
async function main() {
  ...
  await checkPage("/today", [
    ...
    ["connector rows show no synthetic clock", `(() => { const ol = document.querySelector('ol'); if (!ol) return true; const text = ol.innerText; return !/\\d{2}:\\d{2}/.test(text) && text.includes('—'); })()`],
  ]);
